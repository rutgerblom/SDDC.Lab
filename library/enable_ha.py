...
def main():
    ...
    cluster_config_spec.dasConfig = configure_ha()
    task = cluster.ReconfigureCluster_Task(cluster_config_spec, True)
    wait_for_tasks([task])
    module.exit_json(changed=True, result="vSphere HA is enabled on cluster " + module.params['cluster'])
