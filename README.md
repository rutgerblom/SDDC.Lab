...
8. In newer versions of the VyOS router nightly builds the setup dialog that is launched when running ```ìnstall image``` has slightly changed. Also the command syntax for configuration of a DHCP service has changed. These changes cause the router deployment to fail in version 6 of SDDC.Lab. We have addressed this in the [dev-v7](https://github.com/rutgerblom/SDDC.Lab/tree/dev-v7) branch of the project. To avoid this issue you either need to place an older VyOS ISO image in your software repository or switch to the dev-v7 branch of this project.
