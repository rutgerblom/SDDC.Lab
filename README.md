...
2. If you attempt to deploy a pod, but receive a message indicating "directory /tmp/Pod-### is not empty", that's because a previous pod deployment failed (for whatever reason), and some files remain in the /tmp/Pod-### directory.  To resolve this issue, delete the /tmp/Pod-### directory, and then re-deploy the Pod.  If an ISO image is still mounted (which you can check by running 'mount'), then you will need to unmount the ISO image before you can delete the /tmp/Pod-### directory.  In all examples, the "###" of Pod-### is the 3-digit Pod Number.
